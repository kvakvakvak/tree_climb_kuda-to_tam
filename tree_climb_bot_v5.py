...
class HoneyGame:
    ...
    def _state_text(self):
        ...
        return (
            "Вы сидите на нижней ветке у дупла. Внутри пахнет медом.\n"
        ...
        if self.fur_type is None:
            return ("Выберите тип шерсти:\n"
                    ...
                    "3 — очень пушистая/пышная"), self._kb([
                        [("1", "primary", "fur_smooth"),
                         ("2", "primary", "fur_fluffy"),
                         ("3", "primary", "fur_very_fluffy")]
                    ])
        ...
        if self.smell is None:
            return ("Запах:\n"
                    "Как сейчас скрыт ваш запах?"), self._kb([
                        ...
                        [("Грязь", "primary", "smell_mud")],
                        [("Смыт водой", "primary", "smell_water")],
                    ])
    ...
    def main_screen(self):
        ...
        rows = [
            [("Зацепить когтем", "primary", "act_scratch"),
             ("Отломить кусок", "negative", "act_break")],
            [("Замереть", "secondary", "act_freeze"),
             ("Спуститься", "primary", "act_down")]
        ]
    ...
    def check_swarm(self):
        ...
        if lost > 0:
            lines.append(f"Часть сот ломается и остаётся под деревом (-{lost} куска).")
        lines.append("")
    ...
    def _ev_nose_bee(self):
        ...
        kb = self._kb([
            [("Смахнуть", ca, "ev_nose_swipe"),
             ("Зажмуриться и ждать", cb, "ev_nose_wait")]
        ])
        return text, kb
    ...
    def resolve_ev_nose_swipe(self):
        ...
        if dmg > 0:
            ...
            lines.append("Жало скользит по шерсти.")
    ...
    def _ev_stuck_paw(self):
        ...
        ca, cb = self._color_pair("b")
        kb = self._kb([
            [("Резко дернуть", ca, "ev_paw_jerk"),
             ("Медленно тянуть", cb, "ev_paw_slow")]
        ])
        return text, kb

    def resolve_ev_paw_jerk(self):
        lines = []
        lines.append("Вы резко дёргаете лапу. Фу, липко.")
    ...
    def resolve_ev_paw_slow(self):
        lines = []
        lines.append("Вы осторожно вытягиваете лапу из улья.")
    ...
    def resolve_ev_piece_catch(self):
        ...
        lines.append(f"Добыча сохранена (+2 соты), но дупло содрогается (+{delta} тревоги).")
        return "\n".join(lines)
    ...
    def resolve_ev_wind_edge(self):
        ...
        if dmg > 0:
            ...
            lines.append("Пчёлы жалят в шерсть, почти не причиняя вреда.")
...
def cmd_honey(peer_id, user_id, args=None):
    ...
    tree_game = active_games.get(peer_id)
    if tree_game and tree_game.phase in ("setup", "playing"):
        return "Сейчас уже идёт лазание по дереву. Сначала завершите его.", None
...
if __name__ == "__main__":
    ...
    for event in longpoll.listen():
        ...
        if event.type == VkBotEventType.MESSAGE_NEW:
            ...
            elif payload.get("game") == "honey":
                response, keyboard = handle_honey_callback(
                    peer_id, user_id, payload)
